...
def is_epoch_signal_bad(epoch_signal, fs, bad_segment_threshold=1):
    '''
    ...
    epoch_signal = butter_bandpass_filter(epoch_signal, 0.5, 35, fs, order=1)
    
    num_bad_segments = 0
    bad_signal_counter = 0
    segment_window_length = int(fs*5) # int(fs*10)
    segment_window_stride = int(fs*5) #int(fs*10)
