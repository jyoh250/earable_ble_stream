    ...
    for i in range(int(gaze_window_len_s*fs), data.shape[0], int(gaze_window_stride_s*fs)):
        if ((i/fs) >= next_sqc_check_ts) and (i >= int(sqc_window_len_s*fs)):
            ...
            if not ((correlation < -0.6) and (window_l_signal_quality == 1) and (window_r_signal_quality == 1)):
                ...
                window_l_sqc_feats = (window_l_sqc_feats-gaze_sqc_feature_means)/gaze_sqc_feature_stds
                
                window_r_sqc_feats = extract_sqc_features(sqc_window_data_r, fs)
                window_r_sqc_feats = (window_r_sqc_feats-gaze_sqc_feature_means)/gaze_sqc_feature_stds
                
                window_l_signal_quality, window_r_signal_quality = 1-(np.round(gaze_sqc_model.predict(np.array([window_l_sqc_feats, window_r_sqc_feats])).flatten()))
